...
const Details = () => {
    ...
    return (
        <div>
            <div className='upperImageDiv relative z-0 w-full'>
            ...
            <div className='lowerDiv pb-20 min-h-[100vh] bg-[#1C1D1F] flex flex-col'>
                <div className="innerDiv w-full lg:w-[70%] mx-auto h-fit rounded-none lg:rounded-xl bg-[#1C1D1F] relative py-10 border border-[#282b30] mt-[-3rem] lg:mt-[-5rem]">
                    <div className='countryImage w-[200px] lg:w-[320px] mx-auto mt-[-5rem]'>
                        <img className='w-full object-contain rounded-lg' src={countryDetails?.flags?.png} alt={countryDetails?.name?.common} />
                    </div>
                    <div>
                        <p className='text-[22px] lg:text-[30px] font-bold text-[#D2D5DA] text-center mt-4'>
                            {countryDetails?.name?.common}
                        </p>

                        <p className='text-[14px] lg:text-[16px] font-semibold text-[#D2D5DA] mt-1 text-center'>
                            {countryDetails?.name?.official}
                        </p>
                    </div>

                    <div className='flex justify-center items-center gap-4 mt-4'>
                        <div className='bg-[#282B30] px-5 py-3 rounded-xl text-[#D2D5DA] font-semibold text-[12px] lg:text-[16px]'>
                    ...
                    {neighboringCountries.length > 0 && (
                        <div className='neighboring-countries mt-10 px-10'>
                            <div className='text-[#6C727F] text-[16px] font-semibold text-center lg:text-left'>
                            ...
                            <div className='flex flex-wrap justify-center lg:justify-start items-center gap-6 mt-2'>
                                {neighboringCountries?.map((neighbor) => (
                                    <div key={neighbor?.cca3} className='neighbor-country'>
                                        <img className='w-[60px] lg:w-[80px] h-[40px] lg:h-[48px] object-contain rounded-lg mb-2' src={neighbor.flags.png} alt={neighbor?.name.common} />
